...
| `taxonomy_version` | string | Taxonomy version under which `themes` were assigned. Format: v3. Set at extraction, or by a versioned re-tag pass. |
...
| `verbatim` | string | One unaltered quote from the transcript. Preserves false starts, fragmentation, and repetition. Selected to represent the dominant register of the beat — not for drama or quotability. May be restored by a versioned re-tag pass when the stored text no longer matches the transcript exactly. See extraction-rules.md. |
...
*Schema v4 · added taxonomy_version. Existing records keep their schema_version. September 25, 2026.*
